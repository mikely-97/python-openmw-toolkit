...
# -- Door activators (teleport + conditional access) ------------------------
# These are ACTI objects shaped like doors; teleportation is handled by Lua.
addon.add_activator("hw_door_garden",
...
addon.add_activator("hw_door_return_garden",
...
addon.add_light("hw_light_ceiling",
    name="", radius=600, color=(220, 200, 180),
    duration=-1, weight=0.0, value=0,
    flags=["dynamic"])

addon.add_light("hw_light_warm",
    name="", radius=350, color=(255, 180, 100),
    duration=-1, weight=0.0, value=0,
    flags=["dynamic", "fire", "flicker"])

addon.add_light("hw_light_garden",
    name="", radius=500, color=(160, 220, 120),
    duration=-1, weight=0.0, value=0,
    flags=["dynamic"])

addon.add_light("hw_light_forest",
    name="", radius=400, color=(120, 180, 100),
    duration=-1, weight=0.0, value=0,
    flags=["dynamic"])

addon.add_light("hw_light_mine",
    name="", radius=250, color=(255, 150, 60),
    duration=-1, weight=0.0, value=0,
    flags=["dynamic", "fire", "flicker"])
...
# ---------------------------------------------------------------------------
# illegal_to_sleep everywhere; bed is an ACTI handled by Lua
...
# Floor and walls (invisible without mesh files — see generate_meshes.py)
hub.place_static("hw_floor_hub",  x=0,    y=0,    z=-10)
hub.place_static("hw_wall_hub",   x=0,    y=2200, z=0,   rot_z=0)       # north
hub.place_static("hw_wall_hub",   x=0,    y=-2200,z=0,   rot_z=0)       # south
hub.place_static("hw_wall_hub",   x=2200, y=0,    z=0,   rot_z=1.5708)  # east
hub.place_static("hw_wall_hub",   x=-2200,y=0,    z=0,   rot_z=1.5708)  # west

# Ceiling light — central
hub.place_light("hw_light_ceiling", x=0, y=0, z=500)

# Decorative pillars at four quadrant corners
for px, py in [(-1500, 1500), (1500, 1500), (1500, -1500), (-1500, -1500)]:
    hub.place_static("hw_pillar", x=px, y=py, z=0)

# Torch lights near pillars
for lx, ly in [(-1300, 1300), (1300, 1300), (1300, -1300), (-1300, -1300)]:
    hub.place_light("hw_light_warm", x=lx, y=ly, z=200)

# Vendor NPC — center of the room
hub.place_npc("hw_vendor", x=0, y=0, z=0, rotation=0)

# Bed — SE alcove
hub.place_activator("hw_bed", x=700, y=-600, z=0)

# Crafting stations — NW area
hub.place_activator("hw_anvil", x=-700, y=600, z=0)
hub.place_activator("hw_forge", x=-950, y=600, z=0)
hub.place_light("hw_light_warm",  x=-850, y=700, z=350)  # forge glow

# Mine Management Board — near south (mine) door
hub.place_activator("hw_mine_refresh_station", x=0, y=-1700, z=0)

# --- Doors ---
# Garden door — north wall, always open
hub.place_activator("hw_door_garden",  x=0,    y=1900, z=0)

# Forest door — east wall, needs Forest Pass
hub.place_activator("hw_door_forest",  x=1900, y=0,    z=0, rot_z=1.5708)

# Mine door — south wall, needs Mine Pass
hub.place_activator("hw_door_mine",    x=0,    y=-1900,z=0)
...
garden.place_light("hw_light_garden",  x=-600, y=-400, z=300)
garden.place_light("hw_light_garden",  x=500, y=400, z=300)

# Return door (south, leads back to Hub World)
garden.place_activator("hw_door_return_garden", x=0, y=-900, z=0)

# Basil plants (5)
BASIL_POS = [(-500, 300), (-100, 600), (250, 400), (450, -100), (-350, -300)]
for i, (bx, by) in enumerate(BASIL_POS, 1):
    garden.place_activator(f"hw_plant_basil_{i:02d}", x=bx, y=by, z=0)

# Mint plants (5)
MINT_POS = [(500, 500), (-200, -500), (350, -400), (-550, -150), (150, -200)]
...
forest.place_light("hw_light_forest",  x=-800, y=-600, z=400)
forest.place_light("hw_light_forest",  x=700, y=700, z=400)
forest.place_light("hw_light_warm",    x=0, y=-1100, z=200)  # near entrance

# Return door (south/entrance)
forest.place_activator("hw_door_return_forest", x=0, y=-1200, z=0)

# Trees (6)
TREE_POS = [(-800, -800), (-200, -600), (500, -700),
             (-900, 200), (300, 300), (700, 800)]
for i, (tx, ty) in enumerate(TREE_POS, 1):
    forest.place_activator(f"hw_tree_{i:02d}", x=tx, y=ty, z=0)

# Healing mushrooms (4)
MUSH_HEAL_POS = [(-600, -200), (200, -400), (600, 100), (-300, 600)]
for i, (hx, hy) in enumerate(MUSH_HEAL_POS, 1):
    forest.place_activator(f"hw_mushroom_heal_{i:02d}", x=hx, y=hy, z=0)

# Vigor mushrooms (4)
MUSH_VIG_POS = [(-700, 400), (100, -800), (700, 500), (-100, 200)]
...
# Torch lights scattered along walls
for lx, ly in [(-600, 400), (600, 400), (-600, -200), (600, -200), (0, 700)]:
    mine.place_light("hw_light_mine", x=lx, y=ly, z=220)

# Return door
mine.place_activator("hw_door_return_mine", x=0, y=850, z=0)

# Iron ore deposits (8)
IRON_POS = [(-500, 400), (-200, 200), (100, 400), (400, 200),
             (-600, -100), (-300, -300), (200, -400), (500, -200)]
for i, (ix, iy) in enumerate(IRON_POS, 1):
    mine.place_activator(f"hw_mineral_iron_{i:02d}", x=ix, y=iy, z=0)

# Stone deposits (4)
STONE_POS = [(-700, 100), (300, -500), (600, 350), (-400, 500)]
